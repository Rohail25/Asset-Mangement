                    ...
                    <form id="captureForm" class="row g-2">
                        ...
                        @foreach ($fields as $f)
                            <div class="{{ $f->type === 'textarea' ? 'col-12' : 'col-md-6' }}">
                                <label class="form-label">{{ $f->label }} @if ($f->required)
                        ...
                        <div class="col-12">
                            <div class="d-flex gap-2">
                                ...
                                <a href="{{ route($ns . 'audit.myRows', $client->id) }}" class="btn btn-outline-secondary">My
                                    Rows</a>
                                {{-- <form method="POST" action="{{ route($ns . 'audit.finishDay', $client->id) }}"
                                    onsubmit="return confirm('Finish day?')">
                                    @csrf
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (json.ok) {
                    form.querySelectorAll('input[type=text], input[type=number], textarea').forEach(i => i
                        .value = '');
