...
Route::prefix('admin')
    ...
    ->group(function () {
        ...
        // Reports config + run + export
        Route::get('/clients/{client}/reports',           [ReportController::class, 'index'])->name('reports.index');
        Route::post('/clients/{client}/reports/run',       [ReportController::class, 'run'])->name('reports.run');
        Route::post('/clients/{client}/reports/export',    [ReportController::class, 'exportCsv'])->name('reports.export');

        // Live dashboard & inline edit
        Route::get('/clients/{client}/reports/live',      [ReportController::class, 'live'])->name('reports.live');
        Route::get('/clients/{client}/reports/row/{row}', [ReportController::class, 'editRow'])->name('reports.row.edit');
        Route::post('/clients/{client}/reports/row/{row}', [ReportController::class, 'updateRow'])->name('reports.row.update');

        // Register ROWS import (data) using an existing heading set
        Route::post('/clients/{client}/register/{register}/rows', [AssetRegisterController::class, 'rowsStore'])
            ->name('register.rows.store');

        // Optional: quick count or purge rows for a heading set
        Route::delete('/clients/{client}/register/{register}/rows', [AssetRegisterController::class, 'rowsDestroy'])
            ->name('register.rows.destroy');
        Route::get ('/admin/reports/clients', [ReportController::class,'chooseClient'])->name('reports.chooseClient');
    });
